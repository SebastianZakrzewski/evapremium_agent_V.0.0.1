...
- **Kod:** `tests/dashboard/App.test.tsx` → `it('shows markers supplied by event-backed fixture instead of message text')`
- **Krytyczność:** high
...
- **Kod:** `tests/dashboard/App.test.tsx` → `it('shows transcript with timed events and amount only for quote_issued')`
