...
- **Kod:** `tests/dashboard/App.test.tsx` → `it('shows the token gate when no dashboard token is stored')`
- **Krytyczność:** high
...
- **Kod:** `tests/dashboard/App.test.tsx` → `it('loads four hypotheses and a violation snippet with bearer auth')`
- **Krytyczność:** high
...
- **Kod:** `tests/dashboard/App.test.tsx` → `it('stores the pasted token for this browser session')`
- **Krytyczność:** medium
