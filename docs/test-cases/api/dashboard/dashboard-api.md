...
Kod: `api/src/dashboard/dashboard-read.spec.ts`  
...
- **Kod:** `api/src/dashboard/dashboard-read.spec.ts` → `it('rejects missing bearer and the studio token')`
...
- **Kod:** `api/src/dashboard/dashboard-read.spec.ts` → `it('rejects dashboard token when it equals the studio token')`
- **Krytyczność:** high
...
- **Kod:** `api/src/dashboard/dashboard-read.spec.ts` → `it('allows only DASHBOARD_ORIGIN, not shop origins')`
- **Krytyczność:** high
...
- **Kod:** `api/src/dashboard/dashboard-read.spec.ts` → `it('does not set ACAO for a shop origin on dashboard paths')`
- **Krytyczność:** high
...
- **Kod:** `api/src/dashboard/dashboard-read.spec.ts` → `it('counts four hypotheses from events, not agent text')`
- **Krytyczność:** critical
...
- **Kod:** `api/src/dashboard/dashboard-read.spec.ts` → `it('lists session markers from event types')`
- **Krytyczność:** high
...
- **Kod:** `api/src/dashboard/dashboard-read.spec.ts` → `it('joins transcript direction/text with timed events')`
- **Krytyczność:** high
...
- **Kod:** `api/src/dashboard/dashboard-read.spec.ts` → `it('orders session events by occurredAt')`
- **Krytyczność:** medium
