...
Kod: `api/src/chat/shop-cors.spec.ts`  
...
- **Kod:** `api/src/chat/shop-cors.spec.ts` → `it('keeps shop origins and appends https widget origin')`
- **Krytyczność:** high
...
- **Kod:** `api/src/chat/shop-cors.spec.ts` → `it('ignores http widget origins')`
- **Krytyczność:** high
