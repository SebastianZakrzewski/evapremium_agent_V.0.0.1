...
Kod: `api/src/mastra/intents/intent-fallback.spec.ts`
...
- **Kod:** `api/src/mastra/intents/intent-fallback.spec.ts` → `it('uses out_of_scope and zero shop tools when confidence stays low')`
- **Krytyczność:** high
...
- **Kod:** `api/src/mastra/intents/intent-fallback.spec.ts` → `it('keeps pricing when reclassify raises confidence')`
- **Krytyczność:** medium
...
- **Kod:** `api/src/mastra/intents/intent-fallback.spec.ts` → `it('maps a missing profile to out_of_scope with no shop tools')`
- **Krytyczność:** high
...
- **Kod:** `api/src/mastra/intents/intent-fallback.spec.ts` → `it('maps a thrown qualifier to out_of_scope')`
- **Krytyczność:** high
...
- **Kod:** `api/src/mastra/intents/intent-fallback.spec.ts` → `it('does not put Bitrix lead on Mastra tools; Nest still requires consent')`
