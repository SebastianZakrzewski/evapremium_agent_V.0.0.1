...
Kod: `tests/api/templates/template-cascade.resolver.spec.ts`  
...
- **Kod:** `tests/api/templates/template-cascade.resolver.spec.ts` → `it('resolves a known vehicle to one template')`
- **Krytyczność:** medium
...
- **Kod:** `tests/api/templates/template-cascade.resolver.spec.ts` → `it('returns none and many from the same catalogs')`
- **Krytyczność:** high
